...
namespace NotifyIconWpf.Interop
{
  ...
  internal class AppBarInfo
  {
    ...
    {
      get { return (ScreenEdge) this.m_data.uEdge; }
    }
    ...
    public void GetPosition(string strClassName, string strWindowName)
    {
      ...
      this.m_data.cbSize = (UInt32) Marshal.SizeOf(this.m_data.GetType());

      IntPtr hWnd = FindWindow(strClassName, strWindowName);

      if (hWnd != IntPtr.Zero)
      {
        UInt32 uResult = SHAppBarMessage(ABM_GETTASKBARPOS, ref this.m_data);
